...
try:
    ...
    print("✅ Llama 3 Client Initialized.")
except Exception as e:
    print(f"⚠️ Failed to init Llama 3: {e}")
# --------------------------
...
# CONFIGURATION
# ==========================================
MODEL_PATH = r"C:\Users\Ashandth\Desktop\R26-SE-040\model"
...
def get_response(text):
    ...
    if not response:
        if chatbot_name.lower() in text:
            ...
            # ==========================================
            # LLAMA 3 HYBRID FALLBACK (Replaces generic responses)
            # ==========================================
            print("\n🧠 [Routing to Llama 3...]")
            
            # 1. Build the system prompt
            llama_messages = [
                {"role": "system", "content": f"You are {chatbot_name}, a friendly, intelligent AI assistant. Keep your answers brief, natural, and conversational. Do not use lists or markdown."}
            ]
            
            # 2. Add recent conversation context (last 6 messages) so Llama has memory
            for role, msg in conversation_history[-6:]:
                # Convert your history format ('user'/'bot') to OpenAI format ('user'/'assistant')
                llama_role = "assistant" if role == "bot" else "user"
                llama_messages.append({"role": llama_role, "content": msg})
                
            try:
                # 3. Call local Llama 3 via Ollama
                llm_response = llama_client.chat.completions.create(
                    model="phi3",
                ...
                print(f"⚠️ Llama 3 connection error: {e}")
                print("💡 Ensure 'ollama run llama3' is active in the background.")
                response = "I'm having a little trouble connecting to my deeper thought engine right now."
